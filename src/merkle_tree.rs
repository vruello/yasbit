...
impl MerkleTree {
    ...
    fn layer_up(elements: Vec<MerkleTreeNode>) -> Vec<MerkleTreeNode> {
        ...
        if odd {
            new_elements.push(MerkleTree::concat(&elements[elements_len-1],
    ...
    fn dump_rec(elements: Vec<MerkleTreeNode>, acc: &mut Vec<MerkleTreeLayer>)
        -> &Vec<MerkleTreeLayer> {
        if elements.len() == 1 {
...
mod tests {
    ...
    fn test_one_elt() {
        ...
        if let Some(hash) = mk.root() {
            assert_eq!(hash, hex::decode(
                    "41f758f2e5cc078d3795b4fc0cb60c2d735fa92cc020572bdc982dd2d564d11b")
                .unwrap().as_slice());
        } else {
            panic!();
        }
    ...
    fn test_two_elts() {
        ...
        if let Some(hash) = mk.root() {
            assert_eq!(hash, hex::decode(
                    "494c9c623bffa28edd2211dc1a9d364fd298f2906c85c5f8947e4a6396cf6472")
                .unwrap().as_slice());
        } else {
            panic!();
        }
    ...
    fn test_three_elts_and_layers() {
        ...
        if let Some(hash) = mk.root() {
            assert_eq!(hash, hex::decode(
                    "1225b763f8a06c508bd7c0551c09d090d1e50944ee4bab5b78b7ccd0fa9e4c9f")
                .unwrap().as_slice());
        } else {
            panic!();
        }
        ...
        assert_eq!(layers[0].elements[0], hex::decode(
                "41f758f2e5cc078d3795b4fc0cb60c2d735fa92cc020572bdc982dd2d564d11b")
            .unwrap().as_slice());

        assert_eq!(layers[0].elements[1], hex::decode(
                "f9e00e3113f3bfd7653e049d899e5f3c917d020780128ff686e37ce215ab74fe")
            .unwrap().as_slice());
        assert_eq!(layers[0].elements[2], hex::decode(
                "9953051d0daf36399447027f1ff4ceee27161c808c610b3f961ea3805ab3e793")
            .unwrap().as_slice());
        
        assert_eq!(layers[1].elements.len(), 2);
        assert_eq!(layers[1].elements[0], hex::decode(
                "494c9c623bffa28edd2211dc1a9d364fd298f2906c85c5f8947e4a6396cf6472")
            .unwrap().as_slice());
        assert_eq!(layers[1].elements[1], hex::decode(
                "b1609f84ed2489e94bc3eddc66875f8a38b7aab83c5f9a09875fe41f29132350")
            .unwrap().as_slice());
        
        assert_eq!(layers[2].elements.len(), 1);
        assert_eq!(layers[2].elements[0], hex::decode(
                "1225b763f8a06c508bd7c0551c09d090d1e50944ee4bab5b78b7ccd0fa9e4c9f")
            .unwrap().as_slice());
    }
    ...
    #[test]
    fn test_height() {
        let mk1 = MerkleTree::new(&vec![Box::new(1), Box::new(2), Box::new(3),
            Box::new(4), Box::new(5), Box::new(6), Box::new(7)]);
        assert_eq!(mk1.height(), 4);
        let mk2 = MerkleTree::new(&vec![Box::new(1), Box::new(2), Box::new(3),
            Box::new(4), Box::new(5), Box::new(6), Box::new(7), Box::new(8)]);
        assert_eq!(mk2.height(), 4);
        let mk3 = MerkleTree::new(&vec![Box::new(1), Box::new(2), Box::new(3),
            Box::new(4), Box::new(5), Box::new(6), Box::new(7), Box::new(8),
            Box::new(9)]);
