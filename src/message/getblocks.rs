...
impl message::MessageCommand for MessageGetBlocks {
    ...
    fn length(&self) -> u32 {
        let block_locator_len_size = VariableInteger::new(
            self.block_locator_hashes.len() as u64)
            .bytes()
            .len();
        (4 + block_locator_len_size + self.block_locator_hashes.len() * 4 +
         4) as u32
    }
    ...
    fn bytes(&self) -> Vec<u8> {
        ...
        let block_locator_size = VariableInteger::new(
            self.block_locator_hashes.len() as u64);
...
impl MessageGetBlocks {
    ...
    /// of block height.
    pub fn new(version: u32, block_locator_hashes: Vec<crypto::Hash32>,
               hash_stop: crypto::Hash32) -> Self {
        MessageGetBlocks {
            version,
...
mod tests {
    ...
    fn test_message_get_blocks_empty() {
        ...
        assert_eq!(getblocks.block_locator_hashes, vec![] as Vec<crypto::Hash32>);
