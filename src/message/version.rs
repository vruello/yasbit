...
impl message::MessageCommand for MessageVersion {
    ...
    fn length(&self) -> u32 {
        let user_agent_len_size = VariableInteger::new(
            self.user_agent.len() as u64)
            .bytes()
            .len();
        (4 + 8 + 8 + 26 + 26 + 8 + user_agent_len_size +
         self.user_agent.len() + 4) as u32
    }
    ...
    fn bytes(&self) -> Vec<u8> {
        ...
        let user_agent_length = VariableInteger::new(
            self.user_agent.len() as u64);
...
impl MessageVersion {
    fn new(version: u32, services: u64, timestamp: u64,
           addr_recv: network::NetAddrVersion,
           addr_from: network::NetAddrVersion,
           nonce: u64, user_agent: String, start_height: u32) -> Self {
...
mod tests {
    ...
    fn test_message_version() {
        ...
        let message = MessageVersion::new(
            ...
            network::NetAddrVersion::new(message::NODE_NETWORK,
                                         addr.to_ipv6_mapped(),
                                         0),
            network::NetAddrVersion::new(message::NODE_NETWORK,
                                         addr.to_ipv6_mapped(),
